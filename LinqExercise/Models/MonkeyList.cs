...
namespace LinqExercise.Models
{
    class MonkeyList
    {
        ...
        public Monkey SearchMonkeyByName(string name)
        {
            ...
            return this.Monkeys.SingleOrDefault(x => x.Name == name);
        }
        //2
        public List<Monkey> GetAllMonkeysPerLocation(string location)
        {
            return this.Monkeys.Where(x => x.Location == location).ToList();
        }
        //3
        public bool IsThereMonkeyInThatLocation(string location)
        {
            return this.Monkeys.Any(x => x.Location == location);
        }
        //4
        public List<Monkey> SortByLocattionAndName()
        {

            this.Monkeys.Order(x => x.name).ThenBy(x => x.location).ToList();

        }
        //5
        public Monkey SearchMonkeyByNameQuery(string name)
        {
            var mon = from x in Monkeys
                      where x.Name == name
                      select x;
        }
        //6
        ...
        {
            var mon = from x in Monkeys
                      orderby x.Location ascending, x.Name ascending
                      select x;
        ...
        {
            var mon = this.Monkeys.GroupBy(x => x.Location).ToList();
            foreach (var monkey in mon)
            {
                Console.WriteLine(x.key + x.Element);
            }
        }
        //9
        public void PrintNumberOfMonkeysPerLocationQuery()
        {
            var mon = from =in this.Monkeys

                group x by = x.location into g;


            select(Key = g.key, items = g.ToList());
